...
public class Pathfinding_Algorithm {
	...
	public static ArrayList<Zone> find_path(Zone start, Zone destination, BufferedImage map) {
		...
		for (int counter = 0, moves_without_obstacle = 10; current_zone.getDistance_to_destination() != 0
				&& counter < 4000; counter++, moves_without_obstacle++) {

			// mark as visited
			current_zone.setComputed(true);

			/* zone is not passable */
			if (!current_zone.isPassable(map)) { // go back a zone
				current_zone = current_zone.getZone_arrived_from();
				moves_without_obstacle = 0;
				continue;
			}
			...
			if (null == next_zone) { // go back a zone
				current_zone = current_zone.getZone_arrived_from();
				continue;
			}
	...
	private static double calculate_standard_deviation(ArrayList<Zone> locations) {
		// instead of finding difference of average this takes the first
		// location
	...
	// recursive version. causes a StackOverflowException when path gets too
	// long

	/*
	 * public static ArrayList<Zone> take_step(ArrayList<Zone> path_history,
	 * Zone[][] zones_visited, Zone current_zone, Zone destination,
	 * BufferedImage map, int counter, int waypoint_cooldown, int
	 * parameter_cooldown) {
	 * 
	 ...
	 * (!current_zone.isPassable(map)) { // create a waypoint that we get points
	 * for moving away from if (waypoint_cooldown <= 0) { waypoint_cooldown =
	 * 400; Image_Data_Scanner.add_waypoint(current_zone.getLocation()); }
	 * 
	 ...
	 * parameter_cooldown); }
	 * 
	 * System.out.println("3 creating neighbors"); // create or set the
	 * neighbours of the current zone try {
	 * current_zone.create_neighbor_zones(zones_visited, destination);
	 * current_zone.sort_neighbours(); } catch (Error e) { e.printStackTrace();
	 * }
	 * 
	 * System.out.println("4 chosing closest neighbor"); // chose the next
	 * closest neighbor to destination to move to
	 * 
	 * Zone next_zone = current_zone.get_next_best_neighbour();
	 * 
	 * // if all neighbors have been visited go back one zone if (null ==
	 * next_zone) { System.out.println("NO NEIGHBORS LEFT. STEP BACK"); Zone
	 * last_zone = current_zone.getZone_arrived_from(); return
	 * take_step(path_history, zones_visited, last_zone, destination, map,
	 ...
	 * 
	 * // mark the new zone as visited next_zone.setComputed(true); // add new
	 * zone to the path history path_history.add(next_zone);
	 * 
	 ...
	 * }
	 * 
	 * System.out.println("6 take new step"); // explore from the new zone
	 * return take_step(path_history, zones_visited, next_zone, destination,
	 * map, counter, waypoint_cooldown, parameter_cooldown);
	 * 
	 * }
	 * 
	 */
